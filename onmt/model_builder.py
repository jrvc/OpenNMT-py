...
def build_base_multilingmodel(model_opt, fields_dict, gpu, checkpoint=None):
    """
    Args:
        model_opt: the option loaded from checkpoint.
        fields: `Field` objects for the model.
    ...
    assert model_opt.model_type in ["text", "img", "audio"], \
        ("Unsupported model type %s" % (model_opt.model_type))
    models = {}
    lang_pairs = list(fields_dict.keys()).reverse()
    for fields in fields_dict:
        ...
        models.update({lang_pairs.pop():model})
    return models
...
def build_model(model_opt, opt, fields, checkpoint):
    ...
    if model_opt.use_attention_bridge:
        model = build_base_multilingmodel(model_opt, fields,
